...
import torch
import music21
from encoderdecoder_model import seq2seq, EncoderRNN, DecoderRNN

CURRENT_PATH = '/Users/tzvikif/Documents/Msc/Deep Learning/Project/Meta/'

#Load Models
toload = torch.load(CURRENT_PATH + '105modelstate.pth', map_location = 'cpu')
...
encoder.load_state_dict(toload['enc_state_dict'])
decoder.load_state_dict(toload['dec_state_dict'])
s2s.load_state_dict(toload['s2s_state_dict'])
...
for song in data_list:
    ...
    for idx, note in enumerate(part1): #for each note
        ...
        length_vec1[part2[idx][128:192].max(0)[1].item()+4] = 1 #Whichever length had highest probability is the one chosen
